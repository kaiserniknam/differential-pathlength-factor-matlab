function [] = evaluate_absolute_recovery
%EVALUATE_ABSOLUTE_RECOVERY Assisted absolute MBLL absorption recovery.
%
% Evaluates the recovery of the absorption coefficient using the modified
% Beer-Lambert law (MBLL) in absolute mode for several DPF models.
% The geometry/scattering term G is approximated by the optical density
% at the shortest available source-detector separation, representing
% G ~= OD(d -> 0).
%
% This is the assisted inversion: for each simulated absorption condition,
% the DPF evaluated at the corresponding true mu_a is supplied directly
% to the MBLL inversion. This isolates the intrinsic accuracy of each DPF
% formulation from errors associated with determining the appropriate DPF.
%
% The script compares constant, semi-infinite, mean-pathlength,
% slope-based, true, inverse-distance, and empirical DPF models and
% evaluates absorption recovery as a function of source-detector distance.
...
% ---- variables
set_of________G = nan(length(set_of_mua),1);
...
for i_a = 1:length(set_of_mua)
    mua = set_of_mua(i_a);
    mus = set_of_mus(1);
    [dist, OD, cnst_DPF, smif_DPF, savg_DPF, slop_DPF, true_DPF, idst_DPF, empr_DPF, G] = get_DPF(mua,mus,dlta_d);
    set_of________G(i_a)                    = G;
...
for i_d = 1:length(dist)
    ...
    for i_od = 1:size(set_of_______OD,1)
        ...
        % constant dpf
        idx = 2;
        set_of_est_muas(idx,i_d,i_od) = solve_equation (dOD, dist, set_of_mua, set_of_cnst_DPF, i_d, i_od); clearvars idx
        % semi-infinite dpf
        idx = 3;
        set_of_est_muas(idx,i_d,i_od) = solve_equation (dOD, dist, set_of_mua, set_of_smif_DPF, i_d, i_od); clearvars idx
        % mean-pathlength dpf
        idx = 4;
        set_of_est_muas(idx,i_d,i_od) = solve_equation (dOD, dist, set_of_mua, set_of_savg_DPF, i_d, i_od); clearvars idx
        % slope-based dpf
        idx = 5;
        set_of_est_muas(idx,i_d,i_od) = solve_equation (dOD, dist, set_of_mua, set_of_slop_DPF, i_d, i_od); clearvars idx
        % true dpf
        idx = 6;
        set_of_est_muas(idx,i_d,i_od) = solve_equation (dOD, dist, set_of_mua, set_of_true_DPF, i_d, i_od); clearvars idx
        % inverse-distance dpf
        idx = 7;
        set_of_est_muas(idx,i_d,i_od) = solve_equation (dOD, dist, set_of_mua, set_of_idst_DPF, i_d, i_od); clearvars idx
        % empirical dpf
        idx = 8;
        set_of_est_muas(idx,i_d,i_od) = solve_equation (dOD, dist, set_of_mua, set_of_empr_DPF, i_d, i_od); clearvars idx
...
title('Evolution of R^2 over d / assited')
...
title('Evolution of Abs. RMSE over d / assited')
grid on, axis([min(dist) max(dist) 0 100])
grid on, axis([0 8 0 500])
...
for i_d = 1:length(set_of_ids)
    ...
    title([{'True vs. Estimated \mu_a / assited'},{['at d = ',num2str(dist(set_of_ids(i_d))),' cm']}])
    %----- Error table -------------------------------------------------------
...
function [dist, OD, DPF_cnst, DPF_smif, DPF_savg, DPF_slop, DPF_true, DPF_idst, DPF_empr,G] = get_DPF (mua, mus, dlta_d)
g = 0.93;
...
function [true_mu] = solve_equation (dOD, dist, set_of_mua, set_of_DPF, i_d, i_od)
true_mu = dOD/dist(i_d)./set_of_DPF(i_od,i_d);
end
