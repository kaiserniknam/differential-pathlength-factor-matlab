function [] = Fig_07_2
% FIG_07_2 - Absolute MBLL analysis using G approximated at d -> 0.
%
...
% G ~= OD(d -> 0).
%
% This is the non-assisted inversion: the absorption coefficient is
% estimated by searching over the available mu_a-dependent DPF values
% rather than supplying the DPF corresponding to the true mu_a.
%
...
for i_d = 1:length(set_of_ids)
    ...
    title([{'True vs. Estimated \mu_a / not assited'},{['at d = ',num2str(dist(set_of_ids(i_d))),' cm']}])
    %----- Error table -------------------------------------------------------
...
d_error = ((dOD/dist(i_d)./set_of_DPF(:,i_d))./set_of_mua.'-1);
[~,idx] = min(abs(d_error));
