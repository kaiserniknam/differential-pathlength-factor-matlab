function [] = Fig_07_4
% FIG_07_4 - Assisted differential MBLL analysis using G approximated at d -> 0.
%
...
% to the differential MBLL inversion. This isolates the intrinsic accuracy
% of each DPF formulation from errors associated with determining the DPF.
%
% The script compares constant, semi-infinite, mean-pathlength,
% slope-based, true, inverse-distance, and empirical DPF models and
% evaluates absorption-change recovery as a function of source-detector distance.
...
for i_d = 1:length(dist)
    ...
    for i_od = 1:size(set_of_______OD,1)
        ...
        % ODm = set_of_______OD(i_od,i_d) - set_of_______OD(i_od,1);
        % OD0 = set_of_______OD(1   ,i_d) - set_of_______OD(1   ,1);
        dOD = ODm - OD0;
        % true mua
        idx = 1;
...
for i_d = 1:length(set_of_ids)
    ...
    title([{'True vs. Estimated \Delta\mu_a / assited'},{['at d = ',num2str(dist(set_of_ids(i_d))),' cm']}])
    %----- Error table -------------------------------------------------------
...
true_mu = (dOD/dist(i_d) + set_of_mua(1)*set_of_DPF(1,i_d))./set_of_DPF(i_od,i_d);
end
