...
% based on version #1, addressing Angelo Sassaroli's comments
...
[OD_sim_0_25,dist_sim_0_25] = get_OD (mu_a,mu_s/4,dlta_d);
[OD_sim_1_00,dist_sim_1_00] = get_OD (mu_a,mu_s/1,dlta_d);
...
for i_c = 1:length(set_of_C)
    [cnst_DPF, smif_DPF, savg_DPF, slop_DPF, true_DPF, idst_DPF, empr_DPF,G] = get_DPF(mu_a,mu_s.*set_of_C(i_c),dist,dlta_d);
...
function [r2] = r_squared_f(x,y,x_hat,y_hat)
x = x(~isnan(y));
y = y(~isnan(y));
